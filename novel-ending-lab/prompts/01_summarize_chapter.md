...
1. 只做摘要、分析和结构化记录。
2. 不输出大段原文，不摘抄连续长句；必要引用必须极短，并改写为分析语言。
3. 不生成小说正文，不续写，不改写原作，不补写对白或场景。
4. 保持客观，区分明确发生的事实、可疑线索和推测。
5. 如果信息不足，请使用空数组、空字符串，或明确写“待后续章节确认”。
6. 输出必须是机器可读取的合法 JSON；不要输出 Markdown、解释文字或代码块。
7. 本提示词可能由 OpenAI 或 DeepSeek 等 OpenAI 兼容聊天补全接口执行；无论供应商如何，输出 schema 和边界要求保持一致。
8. 不要在输出中包含任何 API key、环境变量值或运行配置。
...
- `chapter_text`：本章文本，仅供分析使用；不得在输出中大段复现

## 输出格式

请只输出合法 JSON，不要使用 Markdown 代码块。字段必须完整，字段名不得增删：
